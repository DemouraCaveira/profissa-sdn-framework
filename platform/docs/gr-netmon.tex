...
\begin{itemize}[leftmargin=1.5em]
  \item As mensagens \texttt{metrics} podem ser ingeridas via \texttt{/metrics/samples} no backend FastAPI ou gravadas em JSONL \texttt{raw/metrics_{layer}.jsonl} para backfill.
  \item As camadas mapeiam direto: physical, link, network, transport, application, control, dataplane.
  \item A UI (Monitor/Lab) consome SSE \texttt{/stream/events}; manter coleta contínua em gr-netmon garante preenchimento ao vivo.
\end{itemize}
